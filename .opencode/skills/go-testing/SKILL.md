...
```bash
docker run --rm -v "$PWD/app:/app" -w /app golang:1.27-alpine go test -v -run TestHandlerHealthz ./...
```

## Check coverage

```bash
docker run --rm -v "$PWD/app:/app" -w /app golang:1.27-alpine go test -cover ./...
```
