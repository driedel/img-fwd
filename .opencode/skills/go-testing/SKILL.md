...
```bash
docker run --rm -v "$PWD/app:/app" -w /app golang:1.22-alpine go test -v -run TestHandlerHealthz ./...
```

## Check coverage

```bash
docker run --rm -v "$PWD/app:/app" -w /app golang:1.22-alpine go test -cover ./...
```
