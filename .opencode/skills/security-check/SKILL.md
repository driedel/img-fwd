...
```bash
docker run --rm -v "$PWD/app:/app" -w /app golang:1.25-alpine go list -m all
```

- Verify no unexpected or deprecated dependencies.
- If `govulncheck` is available, run:
  ```bash
  docker run --rm -v "$PWD/app:/app" -w /app golang:1.25-alpine sh -c "go install golang.org/x/vuln/cmd/govulncheck@latest && govulncheck ./..."
  ```
