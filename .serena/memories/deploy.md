...
- Docker build context: repo root (`Dockerfile` copies from `app/`). Base image `darthsim/imgproxy:v4.0.14`; entrypoint `docker/entrypoint.sh` starts imgproxy on `:8080`, then img-fwd.
